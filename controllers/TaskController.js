...
exports.readTask = (req, res) => {
  Task.findById(req.params.taskid, (err, task) => {
    ...
    console.log(`GET: Task.ID: ${req.params.taskid} ${new Date().toString()}`)
    res.status(200).json(task)
  })
}
...
exports.updateTask = (req, res) => {
  Task.findOneAndUpdate(
    ...
    (err, task) => {
      ...
      console.log(`PUT: Task.ID: ${req.params.taskid} ${new Date().toString()}`)
      res.status(200).json(task)
    }
  )
}
...
exports.deleteTask = (req, res) => {
  Task.deleteOne({ _id: req.params.taskid }, (err, task) => {
    ...
    console.log(`DELETE: Task.ID: ${req.params.taskid} ${new Date().toString()}`)
